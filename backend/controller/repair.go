...
func GetRepair(c *gin.Context) {
	...
	id := c.Param("id")
	if err := entity.DB().Preload("Tenant").Preload("Rental").Preload("RoomEquipment").Preload("Rental.Room").Preload("RoomEquipment.Equipment").Raw("SELECT * FROM repairs WHERE id = ?", id).Find(&repairs).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
